...
import StyledEpGenericCustomDateWrapper from './styles';
...
const EpGenericCustomDate = props => {
  const {
    getPConnect,
    value,
    validatemessage,
 ...
 let { readOnly, required, disabled } = props;
  ...
  if (displayMode === 'LABELS_LEFT' || displayMode === 'STACKED_LARGE_VAL' || displayMode === 'DISPLAY_ONLY') {
    ...
    switch(displayMode){
      case 'DISPLAY_ONLY': {
        return ( <StyledEpGenericCustomDateWrapper> displayComp </StyledEpGenericCustomDateWrapper>);
      }
      case "LABELS_LEFT" : {
        return (
          <StyledEpGenericCustomDateWrapper>
          <FieldValueList
  ...
  if (readOnly && showAsFormattedText) {
    ...
    //const locale = environmentInfo && environmentInfo.toLocaleDateString();
...
EpGenericCustomDate.defaultProps = {
  value: undefined,
  validatemessage: "",
  helperText: "",
  hideLabel: false,
...
export default EpGenericCustomDate;
